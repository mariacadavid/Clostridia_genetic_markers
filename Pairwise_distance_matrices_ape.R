...
gene_aln  <- read.alignment(file = "/Users/mariacadavid/Desktop/Tesis_Clostridiales/Data/Alineamientos/Alineamientos_genes_GTDB/genes_dnak1_GTDB_tblastn_aligned_MAFFT.fasta", format = "fasta") #Input alignment
...
gene_aln  <- read.alignment(file = "/Users/mariacadavid/Desktop/Tesis_Clostridiales/Data/Alineamientos/Alineamientos_genes_GTDB/genes_gyrb_GTDB_tblastn_aligned_MAFFT.fasta", format = "fasta") #Input alignment
#dist.dna() function requires the input alignment to be in a special format known as “DNAbin” format, so we must use the as.DNAbin() function
aln_bin <- as.DNAbin(gene_aln) # Convert the alignment to "DNAbin" format
dist_matrix <- dist.dna(aln_bin, model= "K80", as.matrix = TRUE, pairwise.deletion = TRUE) # Calculate the genetic distance matrix
